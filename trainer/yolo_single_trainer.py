...
from yolo.feature_base_squeezenet import squeezenet_body
...
argparser = argparse.ArgumentParser(
    description='Improvement of YOLO framework !')

argparser.add_argument(
    '-d',
    '--data_path',
    help='path to HDF5 file containing pascal voc datum',
...
argparser.add_argument(
    ...
    default='weights/yolo19_weight.h5')

argparser.add_argument(
    '-f',
    '--model_flag',
    help='which model should to train and use (yolo|squeezenet)',
    default='squeezenet')
...
def model_trainer(args):
    ...
    # Create model body.
    if args.model_flag == "yolo":
        model_body = yolo_body(image_input, len(anchors), len(class_names))
    elif args.model_flag == "squeezenet":
        model_body = squeezenet_body(image_input, len(anchors), len(class_names))
    else:
        raise ValueError("No such model {}!".format(args.model_flag))
    ...
    # model.summary()
    # model_pretrain_path = args.model_weight_path
    # model.load_weights(model_pretrain_path, by_name=True)
    ...
    model.fit([image_data, boxes, detectors_mask, matching_true_boxes],
              ...
              epochs=num_steps)
    if args.model_flag == "yolo":
        model.save_weights('weights/yolo19_weight.h5')
    elif args.model_flag == "squeezenet":
        model.save_weights('weights/squeezenet_weight.h5')
    else:
        raise ValueError("No such model {}!".format(args.model_flag))
...
def prediction(args):
    ...
    voc = h5py.File(voc_path, 'r')
    image = PIL.Image.open(io.BytesIO(voc['test/images'][28]))
    orig_size = np.array([image.width, image.height])
    orig_size = np.expand_dims(orig_size, axis=0)
