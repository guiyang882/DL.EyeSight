...
from eagle.observe.base.meta import Augmentor, Sequential, WithChannels
from eagle.parameter import StochasticParameter, Deterministic, Choice, Uniform


class WithColorspace(Augmentor):
    """
...
class ChangeColorspace(Augmentor):
    """
    ...
    def __init__(self, to_colorspace, from_colorspace="RGB", alpha=1.0, name=None, deterministic=False, random_state=None):
        ...
        if eu.is_single_number(alpha):
            ...
            eu.do_assert(len(alpha) == 2,
                         "Expected tuple/list with 2 entries, got %d entries." % (len(alpha),))
        ...
        if eu.is_string(to_colorspace):
            ...
            eu.do_assert(all(
                [eu.is_string(colorspace) for colorspace in to_colorspace]))
            eu.do_assert(all(
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        alphas = self.alpha.draw_samples((nb_images,),
                                         random_state=eu.copy_random_state(random_state))
