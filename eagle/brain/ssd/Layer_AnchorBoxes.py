...
from .box_encode_decode_utils import convert_coordinates
...
class AnchorBoxes(Layer):
    ...
    '''
    def __init__(self, img_height, img_width, this_scale, next_scale,
        aspect_ratios=[0.5, 1.0, 2.0], two_boxes_for_ar1=True, limit_boxes=True,
        variances=[1.0, 1.0, 1.0, 1.0], coords='centroids', normalize_coords=False,
        **kwargs):
        '''
        ...
        if len(variances) != 4:
            raise ValueError("The length of variances must be 4 !")

        variances = np.array(variances)
        if np.any(variances<=0):
            raise ValueError("All variances value must be > 0 !")
