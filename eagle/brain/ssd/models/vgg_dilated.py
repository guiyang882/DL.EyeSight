...
class SSDVGGDilated(Net):
    def __init__(self, common_params, net_params, box_encoder_params):
        super(SSDVGGDilated, self).__init__(common_params, net_params)
        ## 解析common_params
    ...
    def inference(self, images):
        ...
        # # add dilated conv layer
        # filter_tf = tf.Variable(tf.truncated_normal(shape=[3, 3, 64]))
        # conv1_1_dilated = tf.nn.dilation2d(
        #     input=conv1_1,
        #     filter=filter_tf,
        #     strides=(1, 4, 4, 1),
        #     rates=(1, 2, 2, 1),
        #     padding="SAME",
        #     name="conv1_1_dilated")
        # # added over
        ...
        # add dilated conv layer
        filter_tf = tf.Variable(tf.truncated_normal(shape=[3, 3, 128]))
        conv2_2_dilated = tf.nn.dilation2d(
            input=conv2_2,
            filter=filter_tf,
            strides=(1, 4, 4, 1),
            rates=(1, 2, 2, 1),
        ...
        # add dilated conv layer
        filter_tf = tf.Variable(tf.truncated_normal(shape=[3, 3, 256]))
        conv3_3_dilated = tf.nn.dilation2d(
            input=conv3_3,
            filter=filter_tf,
            strides=(1, 4, 4, 1),
            rates=(1, 2, 2, 1),
        ...
        # add dilated conv layer
        filter_tf = tf.Variable(tf.truncated_normal(shape=[3, 3, 512]))
        conv4_3_dilated = tf.nn.dilation2d(
            input=conv4_3,
            filter=filter_tf,
            strides=(1, 4, 4, 1),
            rates=(1, 2, 2, 1),
        ...
        # dealing with the different receptive field feature into single conv layer
        conv4_3_Reduced = tf.layers.Conv2D(
            filters=128,
            kernel_size=(1, 1),
            activation=tf.nn.relu,
            padding='same',
            kernel_initializer=tf.keras.initializers.he_normal(),
            name="conv4_3_Reduced"
        )(conv4_3)
        conv4_3_hyper = tf.concat([conv2_2_dilated, conv4_3_Reduced], axis=3, name='conv4_3_hyper')

        conv5_3_Reduced = tf.layers.Conv2D(
            filters=256,
            kernel_size=(1, 1),
            activation=tf.nn.relu,
            padding='same',
            kernel_initializer=tf.keras.initializers.he_normal(),
            name="conv5_3_Reduced"
        )(conv5_3)
        conv5_3_hyper = tf.concat([conv3_3_dilated, conv5_3_Reduced], axis=3, name='conv5_3_hyper')

        fc7_Reduced = tf.layers.Conv2D(
            filters=512,
            kernel_size=(1, 1),
            activation=tf.nn.relu,
            padding='same',
        ...
        fc7_hyper = tf.concat([conv4_3_dilated, fc7_Reduced], axis=3, name='fc7_hyper')
        # finish dealing

        # Feed conv4_3 into the L2 normalization layer
        conv4_3_norm = tf.nn.l2_normalize(conv4_3_hyper, axis=3, name="conv4_3_norm")
        ...
        conv5_3_mbox_conf = tf.layers.Conv2D(
        ...
        conv9_1_mbox_conf = tf.layers.Conv2D(
            filters=n_boxes_conv9_2 * self.num_classes,
            kernel_size=(3, 3),
            padding='same',
            kernel_initializer=tf.keras.initializers.he_normal(),
            name='conv9_1_mbox_conf'
        )(conv9_1)
        ...
        conv5_3_mbox_loc = tf.layers.Conv2D(
        ...
        conv9_1_mbox_loc = tf.layers.Conv2D(
            filters=n_boxes_conv9_2 * 4,
            kernel_size=(3, 3),
            padding='same',
            kernel_initializer=tf.keras.initializers.he_normal(),
            name='conv9_1_mbox_loc'
        )(conv9_1)
        ...
        conv5_3_mbox_priorbox = AnchorBoxes(
            self.image_height,
            self.image_width,
            ...
            coords=self.coords,
            normalize_coords=self.normalize_coords,
            name='conv5_3_mbox_priorbox')(conv5_3_mbox_loc)
        ...
        conv9_1_mbox_priorbox = AnchorBoxes(
            self.image_height,
            self.image_width,
            ...
            coords=self.coords,
            normalize_coords=self.normalize_coords,
            name='conv9_1_mbox_priorbox')(conv9_1_mbox_loc)
        ...
        conv5_3_mbox_conf_reshape = tf.reshape(
            conv5_3_mbox_conf,
            (-1, self.num_classes),
            name='conv5_3_mbox_conf_reshape')
        ...
        conv9_1_mbox_conf_reshape = tf.reshape(
            conv9_1_mbox_conf,
            (-1, self.num_classes),
            name='conv9_1_mbox_conf_reshape')
        ...
        conv5_3_mbox_loc_reshape = tf.reshape(
            conv5_3_mbox_loc,
            (-1, 4),
            name='conv5_3_mbox_loc_reshape')
        ...
        conv9_1_mbox_loc_reshape = tf.reshape(
            conv9_1_mbox_loc,
            (-1, 4),
            name='conv9_1_mbox_loc_reshape')
        ...
        conv5_3_mbox_priorbox_reshape = tf.reshape(
            conv5_3_mbox_priorbox,
            (-1, 8),
            name='conv5_3_mbox_priorbox_reshape')
        ...
        conv9_1_mbox_priorbox_reshape = tf.reshape(
            conv9_1_mbox_priorbox,
            (-1, 8),
            name='conv9_1_mbox_priorbox_reshape')
        ...
        mbox_conf = tf.concat([
            ...
            conv9_1_mbox_conf_reshape],
            axis=0, name='mbox_conf')

        # Output shape of `mbox_loc`: (batch, n_boxes_total, 4)
        mbox_loc = tf.concat([
            conv4_3_norm_mbox_loc_reshape,
            conv5_3_mbox_loc_reshape,
            fc7_mbox_loc_reshape,
            conv6_2_mbox_loc_reshape,
            conv7_2_mbox_loc_reshape,
            conv9_1_mbox_loc_reshape],
