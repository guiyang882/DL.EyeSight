...
class Net(object):
  ...
  """
    ...
    def _variable_on_cpu(self, name, shape, initializer, pretrain=True,
                         train=True):
    ...
    def conv2d(self, scope, input, kernel_size, stride=1, pretrain=True,
               train=True):
        """convolutional layer
        ...
        with tf.variable_scope(scope) as scope:
            kernel = self._variable_with_weight_decay('weights',
                                                      ...
                                                      pretrain=pretrain,
                                                      train=train)
            conv = tf.nn.conv2d(input, kernel, [1, stride, stride, 1],
                                padding='SAME')
            biases = self._variable_on_cpu('biases', kernel_size[3:],
    ...
    def local(self, scope, input, in_dimension, out_dimension, leaky=True,
              pretrain=True, train=True):
        ...
        with tf.variable_scope(scope) as scope:
            ...
            weights = self._variable_with_weight_decay('weights',
                                                       ...
                                                       pretrain=pretrain,
                                                       train=train)
            biases = self._variable_on_cpu('biases', [out_dimension],
                                           tf.constant_initializer(0.0),
                                           pretrain, train)
